...
from app.api.v1.routers.numbering_ranges import get_numbering_range_service
...
def override_gateway(numbering_range_gateway_mock):
    app.dependency_overrides[get_numbering_range_service] = lambda: (
        numbering_range_gateway_mock
    )
...
def test_get_numbering_ranges(override_gateway):
    ...
    response = client.get(
        ...
        headers={"x-factus-token": "test-token", "Authorization": "Bearer test-jwt"},
    )
...
def test_get_numbering_range_by_id(override_gateway):
    ...
    response = client.get(
        "/api/numbering-ranges/1", headers={"x-factus-token": "test-token"}
    )
    assert response.status_code in [200, 401]
...
def test_create_numbering_range(override_gateway):
    ...
    response = client.post(
        ...
        headers={"x-factus-token": "test-token"},
    )
...
def test_update_numbering_range(override_gateway):
    ...
    override_gateway.update_numbering_range_consecutive.return_value = (
        NumberingRangeResponse(status="OK", message="Success", data=mock_range)
    )
    response = client.put(
        "/api/numbering-ranges/1",
        json={"current": 5},
        headers={"x-factus-token": "test-token"},
    )
    assert response.status_code in [200, 401]
...
def test_get_software_numbering_ranges(override_gateway):
    ...
    override_gateway.get_software_numbering_ranges.return_value = (
        NumberingRangeSoftwareResponse(
            status="OK", message="Success", data=[mock_range]
        )
    )
    response = client.get(
        "/api/numbering-ranges/software", headers={"x-factus-token": "test-token"}
    )
    assert response.status_code in [200, 401]
